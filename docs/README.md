...
- Detektory szyfrowania korzystają z konfigurowalnych sygnatur (BitLocker, VeraCrypt) oraz heurystyki (entropia/statystyki bajtów) jako fallback.
...
- `synthetic_data.md` – strategia tworzenia danych syntetycznych, testowania i oceny jakości + benchmarki (WSL2/Windows).
...
W CLI można ograniczyć detekcję do konkretnych sygnatur parametrem `--signature-id` (można podać wielokrotnie). Bez tego parametru używane są wszystkie sygnatury z konfiguracji.

## Benchmarki i raporty

W repo jest dostępny osobny entry-point benchmarków, który generuje raport skuteczności w JSON i Markdown:

```bash
poetry run crypto-analyzer-benchmark
```

Jeśli chcesz uwzględnić benchmark detekcji systemu plików, przygotuj `test_assets/generated/multi_volume.img` (na Windows bez WSL2: `poetry run python scripts/generate_test_images_windows.py --force`) i uruchom:
