...
- `crypto_detection` – wykrywanie szyfrowania (BitLocker, VeraCrypt) poprzez sygnatury nagłówków oraz heurystyki (np. entropia/statystyki bajtów) jako fallback.
- `benchmarks` – offline benchmarki (sygnatury/magic bytes, heurystyka, opcjonalnie detekcja FS) oraz generowanie raportów (JSON/Markdown).
...
- `crypto_detection.HeuristicEncryptionDetector` – detektor heurystyczny (m.in. entropia/statystyki bajtów), używany jako fallback po detekcji sygnaturowej.
